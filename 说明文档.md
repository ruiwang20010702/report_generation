...
- 语音服务（阿里云）：
  ...
  - `TINGWU_LANGUAGE`
- 数据库（PostgreSQL）：
  - **Zeabur 模式**（优先）：`POSTGRES_CONNECTION_STRING` 或 `DATABASE_URL`（自动注入）
  - **传统模式**：`DB_HOST`、`DB_PORT`、`DB_NAME`、`DB_USER`、`DB_PASSWORD`
  - 连接池配置：`DB_POOL_MAX`、`DB_POOL_IDLE_TIMEOUT`、`DB_POOL_CONNECTION_TIMEOUT`
  - SSL 配置：`DB_SSL`（默认 false，Zeabur 不支持 SSL）、`DB_SSL_REJECT_UNAUTHORIZED`
- 认证/安全：
  - `JWT_SECRET`
- 邮件服务：
...
- **部署状态：✅ 已完成（2025-11-12 15:45）**
- 部署方式：Zeabur Marketplace → PostgreSQL Prebuilt Service
- 环境变量：`POSTGRES_CONNECTION_STRING` 由 Zeabur 自动注入（无需手动配置）
- 初始化脚本：`database/init.sql`（包含 users、otps、reports 三张表）
- 部署文档已创建：
...
- 部署步骤（已完成）：
  ...
  2. ✅ 进入 PostgreSQL Web Console，执行 `database/init.sql`
  3. ✅ 修复代码兼容性问题（支持 Zeabur 连接字符串）
  4. ✅ 推送代码，触发自动重新部署
- **代码修复记录（2025-11-12 15:45-16:00）**：
  - ✅ `server/config/database.ts` - 修复 `testConnection()` 函数，支持连接字符串
  - ✅ `server/config/database.ts` - 禁用 SSL（Zeabur PostgreSQL 不支持 SSL）
  - ✅ `scripts/test-database.ts` - 添加连接字符串模式显示
  - ✅ `scripts/setup-database.ts` - 添加连接字符串模式显示
  - ✅ `scripts/check-env.ts` - 添加 `POSTGRES_CONNECTION_STRING` 检查
  - ✅ 说明文档 - 更新环境变量清单，标注 Zeabur 模式优先
- 验证方法：
  - 应用日志显示 "✅ 数据库连接成功"
  - 健康检查接口返回 `"database": "connected"`
  - 测试用户注册流程（发送验证码 → 验证 OTP）
- 故障排查经验：
  - ✅ 已解决：`testConnection()` 只使用单独环境变量，无法识别 Zeabur 连接字符串
  - ✅ 已解决：脚本日志不显示连接字符串模式
  - ✅ 已解决：SSL 连接错误 - Zeabur PostgreSQL 不支持 SSL，已默认禁用
