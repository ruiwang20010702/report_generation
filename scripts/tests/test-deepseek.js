/**
 * 🔷 DeepSeek API 测试脚本
 * 
 * DeepSeek 特点：
 * - 国内直连，无需 VPN
 * - 超高性价比（比 OpenAI 便宜 95%）
 * - 强大推理能力，适合复杂分析任务
 * 
 * 官网：https://platform.deepseek.com/
 */

import OpenAI from 'openai';
import dotenv from 'dotenv';

dotenv.config();

async function testDeepSeek() {
  console.log('\n' + '='.repeat(70));
  console.log('🔷 DeepSeek API 连接测试');
  console.log('='.repeat(70) + '\n');

  const apiKey = process.env.DEEPSEEK_API_KEY;

  if (!apiKey) {
    console.log('❌ 未配置 DEEPSEEK_API_KEY');
    console.log('\n📝 配置步骤：');
    console.log('   1. 访问 https://platform.deepseek.com/');
    console.log('   2. 注册/登录账号（支持微信/手机）');
    console.log('   3. 获取 API Key');
    console.log('   4. 在 .env 文件中添加：');
    console.log('      DEEPSEEK_API_KEY=sk-你的API密钥');
    console.log('\n💰 定价：');
    console.log('   - 输入：¥1 / 1M tokens');
    console.log('   - 输出：¥2 / 1M tokens');
    console.log('   - 新用户送 500万 tokens 免费额度');
    return;
  }

  console.log('✅ API Key 已配置');
  console.log(`   Key: ${apiKey.substring(0, 10)}...${apiKey.substring(apiKey.length - 4)}\n`);

  try {
    const startTime = Date.now();
    
    const client = new OpenAI({
      apiKey: apiKey,
      baseURL: 'https://api.deepseek.com/v1'
    });

    console.log('🔌 正在连接 DeepSeek API...');
    
    const response = await client.chat.completions.create({
      model: 'deepseek-chat',
      messages: [
        {
          role: 'system',
          content: '你是一位专业的英语教学专家。'
        },
        {
          role: 'user',
          content: '请简短介绍一下你的能力，50字以内。'
        }
      ],
      max_tokens: 200
    });

    const elapsed = ((Date.now() - startTime) / 1000).toFixed(2);

    console.log('\n✅ DeepSeek API 连接成功！\n');
    console.log('📊 测试结果：');
    console.log(`   ⏱️  响应时间: ${elapsed} 秒`);
    console.log(`   📝 模型: ${response.model}`);
    console.log(`   🎯 完成原因: ${response.choices[0].finish_reason}`);
    console.log(`   💬 回复内容: ${response.choices[0].message.content}\n`);

    if (response.usage) {
      console.log('💰 Token 使用：');
      console.log(`   输入: ${response.usage.prompt_tokens} tokens`);
      console.log(`   输出: ${response.usage.completion_tokens} tokens`);
      console.log(`   总计: ${response.usage.total_tokens} tokens`);
      
      // 计算成本（CNY）
      const inputCost = (response.usage.prompt_tokens / 1000000) * 1;
      const outputCost = (response.usage.completion_tokens / 1000000) * 2;
      const totalCost = inputCost + outputCost;
      console.log(`   💵 成本: ¥${totalCost.toFixed(6)} (输入: ¥${inputCost.toFixed(6)} + 输出: ¥${outputCost.toFixed(6)})\n`);
    }

    console.log('✨ 特性：');
    console.log('   ✅ 国内直连（无需 VPN）');
    console.log('   ✅ 超高性价比（比 GPT-4 便宜 95%）');
    console.log('   ✅ 强大推理能力');
    console.log('   ✅ 支持 32K 上下文');
    console.log('   ✅ 新用户送 500万 tokens 免费额度\n');

    console.log('='.repeat(70));
    console.log('🎉 测试完成！DeepSeek 可以正常使用');
    console.log('='.repeat(70) + '\n');

  } catch (error) {
    console.log('\n❌ DeepSeek API 调用失败\n');
    
    if (error.response) {
      console.log('📋 错误详情：');
      console.log(`   状态码: ${error.response.status}`);
      console.log(`   错误信息: ${JSON.stringify(error.response.data, null, 2)}`);
    } else {
      console.log('📋 错误信息：', error.message);
    }

    console.log('\n🔧 排查建议：');
    console.log('   1. 检查 API Key 是否正确');
    console.log('   2. 确认账户是否有余额/免费额度');
    console.log('   3. 检查网络连接是否正常');
    console.log('   4. 查看 DeepSeek 控制台是否有错误提示');
  }
}

testDeepSeek();

