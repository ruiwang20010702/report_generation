  ...
  "grammarExamples": [
    {
      ...
      "incorrect": "学生实际说出的英文错误句子",
      "correct": "正确的英文句子",
      "explanation": "中文错误解释和语法规则说明"
    }
  ]
}
...
**五、grammarExamples 字段（必须提供3个语法错误示例）：**
- 从学生的发言中提取 **3个真实的语法错误**
- 每个示例必须包含：
  1. category：**中文**错误类别（如：主谓一致、动词时态、冠词遗漏、介词使用、单复数错误、词序错误等）
  2. incorrect：学生**实际说出的错误句子**（英文，必须是转录文本中真实出现的！）
  3. correct：正确的句子（英文）
  4. explanation：**中文**错误解释和语法规则说明
- 🔴 **incorrect 必须是学生在对话中真实说过的句子**，不能编造！
- 🔴 **category 和 explanation 必须是中文**，incorrect 和 correct 是英文
- 如果学生语法很好，错误不足3个，可以提供2-3个，但至少要有2个
- 如果实在找不到语法错误，可以指出学生句子中可以改进的地方（如更地道的表达）
