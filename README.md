...
- **Zeabur**（推荐生产）：参考 `QUICKSTART_ZEABUR.md`，自动注入数据库连接串，`npm run build && npm run start` 即可运行。
- **阿里云容器 / ECS**：依赖 `ALIYUN_*`、`GLM_API_KEY` 等环境变量，确保开放 3001 端口并配置 HTTPS。
- HTTP 反向代理（如 Nginx）需将前端静态资源指向 `dist/`，API 透传至 `node build/server/index.js`。
